 ...
 * 
 * Maps Pokemon TCG sets to their respective eras based on sets.set_series and releaseDate.
 */

import type { Era, CardInput } from './types'
...
const SET_ID_ERA_OVERRIDES: Record<string, Era> = {
  // Special sets that don't follow normal set_series naming
  ...
  "sv3pt5": "Scarlet & Violet",  // 151 sets (S&V era)
...
function getEraByDate(releaseDate: string): Era {
  const set_release_date = new Date(releaseDate)
  const year = set_release_date.getFullYear()
  const month = set_release_date.getMonth() + 1
 ...
 */
export function detectEra(cards: CardInput): Era {
  const { sets } = cards
  
  // Try set_series-based detection first (most reliable)
  if (SERIES_ERA_MAP[sets.set_series]) {
    return SERIES_ERA_MAP[sets.set_series]
  }
  
  // Check sets ID overrides for special cases
  if (SET_ID_ERA_OVERRIDES[sets.set_id.toLowerCase()]) {
    return SET_ID_ERA_OVERRIDES[sets.set_id.toLowerCase()]
  }
  
  // Fall back to set_release_date-based detection
  return getEraByDate(sets.releaseDate)
}

/**
 * Check if sets supports reverse holo by default
 */
export function hasReverseHoloDefault(era: Era, releaseDate: string): boolean {
  ...
  if (era === "WotC") {
    ...
    return set_release_date >= legendaryCollectionDate
  }
...
/**
 * Check if sets/era combination supports special pattern variants
 */
